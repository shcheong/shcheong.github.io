...
c:c.toString()};a.readBinary=function(b){b=a.read(b,!0);b.buffer||(b=new Uint8Array(b));n(b.buffer);return b};a.load=function(b){H(read(b))};a.thisProgram||(a.thisProgram=1<process.argv.length?process.argv[1].replace(/\\/g,"/"):"unknown-program");a.arguments=process.argv.slice(2);"undefined"!==typeof module&&(module.exports=a);process.on("uncaughtException",function(b){if(!(b instanceof O))throw b;});a.inspect=function(){return"[Emscripten Module object]"}}else if(G)a.print||(a.print=print),"undefined"!=
